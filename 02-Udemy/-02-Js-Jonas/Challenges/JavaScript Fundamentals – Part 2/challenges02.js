...
const checkWinner = function (averageDolphins, averageKoalas) {
  if (averageDolphins >= averageKoalas * 2) {
    ...
    return console.log(`There is no winner today...`);
  }
};
...
const bill2 = 555;
const bill3 = 44;
let tips = [calcTip(bill1), calcTip(bill2), calcTip(bill3)];
console.log(tips);
...
// Instructor solution (looks shorter :))
...
for (let i = 0; i < bills4.length; i++) {
  // tips4.push(calcTip(bills4[i]));
  // total4.push(bills4[i] + calcTip(bills4[i]));
  let tip = calcTip(bills4[i]);
  tips4.push(tip);
...
console.log(calcAverage4([2, 3, 6])); // 3.66
console.log(calcAverage4(total4));
console.log(calcAverage4(tips4));
